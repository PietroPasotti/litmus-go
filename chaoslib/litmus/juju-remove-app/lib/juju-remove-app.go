...
import (
	...
	experimentTypes "github.com/litmuschaos/litmus-go/pkg/juju/remove-app/types"
...
// PrepareJujuRemoveApp contains the preparation and injection steps for the experiment
func PrepareJujuRemoveApp(ctx context.Context, experimentsDetails *experimentTypes.ExperimentDetails, clients clients.ClientSets, resultDetails *types.ResultDetails, eventsDetails *types.EventDetails, chaosDetails *types.ChaosDetails) error {
	...
	default:
		...
		log.Infof("[Chaos]: Removing application %s", experimentsDetails.AppName)
		if err := jujuOps.RemoveApp(ctx, client, experimentsDetails.AppName); err != nil {
			return stacktrace.Propagate(err, "failed to remove application")
		}
...
func abortWatcher(experimentsDetails *experimentTypes.ExperimentDetails, chaosDetails *types.ChaosDetails) {
	...
	log.Info("[Abort]: No automatic revert for remove-app; manual intervention may be needed")
	common.SetTargets(experimentsDetails.AppName, "reverted", "JujuApp", chaosDetails)
